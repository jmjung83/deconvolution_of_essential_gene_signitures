...
if __name__ == '__main__':
    spearman_files = glob.glob('result/spearman_STOP/all_*.txt')
    
    maxVal_dic = {}
    
    for sp_file in spearman_files:
        sp_df = pd.read_table(sp_file, sep='\t', index_col = 0)
        cell = sp_file[len('result/spearman_STOP/all_'):-len('_24H.txt')]
        if cell == 'HT29': continue
        if cell == 'PC3': continue
        if cell == 'A549': continue
        
        gene = sp_df['ES'].idxmax()
        val = sp_df['ES'].max()
        maxVal_dic[cell] = [gene, val]
    
    
    for cell, val in sorted(maxVal_dic.items(), key=lambda i: i[1][1], reverse=True):
        print cell, val
    
    plotCnt = 0
    for cell, (gene, val) in sorted(maxVal_dic.items(), key=lambda i: i[1][1], reverse=True):
